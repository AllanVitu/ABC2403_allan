package introduction;

public class App {

	public static void main(String[] args) {
		
		// Affiche "Hello world" dans la console
		System.out.println("Hello World !");
		
		
		// VARIABLES
		String prenom; // Déclaration du'une variable ""prenom de type " chaine de caracteres
		
		int age; // Déclaration d'une variable "age" de type "Nombre entier" (valeur scolaire)
		// (plage de valeur d' INT - (2 147 000 000 à +2 147 000 000)
		
		
		long soldeBancaire; // Déclaration d'une varaible "soldeBancaire" de type "Nombre entier 64 bits" (valeur scolaire)
		
		
		float tailleEnMetres; // Déclaration d'une variable "tailleEnMetres" de type "Nombre réel 64 bits" (valeur scolaire)
		
		
		double tailleEnMetresPourLesTresGrands; // Déclaration d'une variable "tailleEnMetresPourLesTresGrands" de type "Nombre réel 64 bits" (valeur scolaire)
		
		
		Boolean estMajeur; //Déclaration d'une variable "estMajeur" de type "Booléen" (valeur scolaire)
		
		
		
		prenom = "Allan"; // Affectation d'une valeur à la variable "prenom"
		
		System.out.println("Salut " + prenom);
		
		prenom = "toto"; // modifcation de la valeur de la variable "prenom"
		
		System.out.println("Bonjour " + prenom);
		
		
		// premierNombre = sc.nextInt(); = interraction avec l'interface chaise clavier pour ecrire un nombre.
		// prenom = sc.nextLine(); = interraction avec l'interface chaise clavier pour ecrire un chaine de crarractères
		
	}

}
